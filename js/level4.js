...
create: function() {
    ...
    var border = platforms.create(290, 130, 'paddle-vert2');
    border.body.immovable = true;
    border.scale.setTo(1, 1);
    border = platforms.create(308, 130, 'paddle-vert2_p_l');
    border.body.immovable = true;
    border.scale.setTo(1, 1);

    border = platforms.create(172, 130, 'paddle-vert2');
    border.body.immovable = true;
    border.scale.setTo(1, 1);
    border = platforms.create(110, 130, 'paddle-vert2_p_r');
    border.body.immovable = true;
    border.scale.setTo(1, 1);

    border = platforms.create(60, 30, 'paddle-vert2_top');
    border.body.immovable = true;
    border.scale.setTo(1, 1);
    border = platforms.create(395, 30, 'paddle-vert2_top');
    border.body.immovable = true;
    border.scale.setTo(1, 1);
                    /*кнопка старт*/ 
    ...
    this.initBricks();
                   /*счёт игры*/ 
    scoreText = this.add.text(15, 7, 'Level 4', { font: '15px Arial', fill: 'white' });
                   /*счёт попыток*/ 
...
initBricks: function() {
    ...
    for(c=0; c<brickInfo.count.col; c++) {
        for(r=0; r<brickInfo.count.row; r++) {
            ...
            newBrick = this.add.sprite(brickX, brickY, 'bricks4');
    ...
    for(c=0; c<brickInfo2.count.col; c++) {
        for(r=0; r<brickInfo2.count.row; r++) {
            ...
            newBrick = this.add.sprite(brickXcol2, brickYcol2, 'bricks4');
            this.physics.enable(newBrick, Phaser.Physics.ARCADE);
            newBrick.body.immovable = true;
            newBrick.anchor.set(0.5);
            bricks.add(newBrick);
    ...
    for(c=0; c<brickInfo3.count.col; c++) {
        for(r=0; r<brickInfo3.count.row; r++) {
            ...
            newBrick = this.add.sprite(brickXcol3, brickYcol3, 'bricks4');
            this.physics.enable(newBrick, Phaser.Physics.ARCADE);
            newBrick.body.immovable = true;
            newBrick.anchor.set(0.5);
            bricks.add(newBrick);
