...
Ball.level9.prototype = {
create: function() {

    this.physics.startSystem(Phaser.Physics.ARCADE);
    this.add.sprite(0, 0, 'fon_l_9');
    ...
    var border = platforms.create(0, 165, 'paddle');
    border.body.immovable = true;
    border.scale.setTo(1, 1);

    border = platforms.create(0, 50, 'paddle');
    border.body.immovable = true;
    border.scale.setTo(1, 1);

    border = platforms.create(375, 165, 'paddle');
    border.body.immovable = true;
    border.scale.setTo(1, 1);

    border = platforms.create(375, 50, 'paddle');
    border.body.immovable = true;
    border.scale.setTo(1, 1);

    border = platforms.create(270, 40, 'paddle-vert2');
    border.body.immovable = true;
    border.scale.setTo(1, 1.3);

    border = platforms.create(190, 40, 'paddle-vert2');
    border.body.immovable = true;
    border.scale.setTo(1, 1.3);
                    /*отбойник*/ 
    ...
    this.initBricks();
                   /*счёт игры*/ 
    scoreText = this.add.text(15, 7, 'Level 9', { font: '15px Arial', fill: 'white' });
                   /*счёт попыток*/ 
...
initBricks: function() {
    ...
    for(c=0; c<brickInfo.count.col; c++) {
        for(r=0; r<brickInfo.count.row; r++) {
            ...
            newBrick = this.add.sprite(brickX, brickY, 'bricks9');
